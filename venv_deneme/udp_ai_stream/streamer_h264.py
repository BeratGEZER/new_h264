...
class StreamManager:
    ...
    def stream_video(self, video_id, stop_flag):
        ...
        stream = output.add_stream('libx264', rate=fractions.Fraction(fps).limit_denominator())
        ...
        MAX_UDP_SIZE = 8000  # sabit paket boyutu
        ...
        while cap.isOpened() and not stop_flag.is_set():
            ...
            for packet in packets:
                ...
                for i in range(0, len(data), MAX_UDP_SIZE):
                    ...
                    if i + MAX_UDP_SIZE >= len(data):
                        chunk += b'__FRAME_END__'
                    sock.sendto(chunk, (udp_ip, udp_port))
                    gönderilen_paket_sayısı += 1
                    gönderilen_toplam_byte += len(chunk)
                    frame_packet += 1
                    frame_byte += len(chunk)

            # ✅ Frame sonunda log
            print("[*************************-------------------------------------****************************")
            print(f"[Streamer {udp_port}] 🎬 Frame gönderildi → Paket: {frame_packet}, Veri: {frame_byte / 1024:.2f} KB")
            print(f"🔸 Toplam Gönderilen Paket: {gönderilen_paket_sayısı}")
            print(f"🔸 Toplam Gönderilen Veri: {gönderilen_toplam_byte / 1024:.2f} KB ≈ {gönderilen_toplam_byte / (1024*1024):.2f} MB")
        ...
        for packet in stream.encode():
            final_data = bytes(packet) + b'__FRAME_END__'
            sock.sendto(final_data, (udp_ip, udp_port))
            gönderilen_paket_sayısı += 1
            gönderilen_toplam_byte += len(final_data)

        cap.release()
        sock.close()
        output.close()

        print(f"\n📦 [Streamer {udp_port}] Yayın Özeti:")
        print(f"🔸 Toplam Gönderilen Paket: {gönderilen_paket_sayısı}")
        print(f"🔸 Toplam Gönderilen Veri: {gönderilen_toplam_byte / 1024:.2f} KB ≈ {gönderilen_toplam_byte / (1024*1024):.2f} MB")
